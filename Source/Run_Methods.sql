...
BEGIN
    ...
    DECLARE @TestEndTime DATETIME = NULL;
    BEGIN TRY
...
BEGIN
    ...
    SELECT ROW_NUMBER() OVER(ORDER BY Result DESC, Name ASC) No,Name [Test Case Name],CAST(DATEDIFF(MILLISECOND,TestStartTime,TestEndTime) AS VARCHAR(7)) AS [Dur(ms)], Result
